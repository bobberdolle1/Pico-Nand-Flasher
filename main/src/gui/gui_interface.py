    ...
    def create_widgets(self):
        ...
        def _toggle_protocol():
            try:
                ...
                pass
        settings_menu.add_checkbutton(label=i18n.t('binary_protocol'), onvalue=True, offvalue=False, variable=self.binary_var, command=_toggle_protocol)
        ...
        def _toggle_oob():
            try:
                ...
                pass
        settings_menu.add_checkbutton(label='Include OOB in dumps', onvalue=True, offvalue=False, variable=self.oob_var, command=_toggle_oob)
        ...
        def _toggle_ecc():
            try:
                ...
                pass
        settings_menu.add_checkbutton(label='Enable ECC verification', onvalue=True, offvalue=False, variable=self.ecc_var, command=_toggle_ecc)

        # ECC parameters dialog
        def _open_ecc_params():
            win = tk.Toplevel(self.root)
            win.title('ECC Parameters')
            tk.Label(win, text='ECC scheme (none|crc16|hamming_512_3byte)').grid(row=0, column=0, sticky='w')
    ...
    def detect_nand(self):
        ...
        if detected and model:
            ...
            try:
                ...
                if op == 'READ':
                    self.resume_label.config(text=i18n.t('resume_mode_read').replace('{page}', str(resume.get('last_page', 0))))
                elif op == 'WRITE':
                    self.resume_label.config(text=i18n.t('resume_mode_write').replace('{bytes}', str(resume.get('bytes_sent', 0))))
                elif op == 'ERASE':
                    self.resume_label.config(text=i18n.t('resume_mode_erase').replace('{block}', str(resume.get('erase_block', 0))))
                else:
    ...
    def read_nand(self):
        ...
        try:
            ...
            if resume.get('operation') == 'READ':
                if not messagebox.askyesno(i18n.t('resume_found_title'), i18n.t('resume_read_text')):
                    self.controller.clear_resume_state()
        except Exception:
            pass
    ...
    def write_nand(self):
        ...
        try:
            ...
            if resume.get('operation') == 'WRITE':
                if not messagebox.askyesno(i18n.t('resume_found_title'), i18n.t('resume_write_text')):
                    self.controller.clear_resume_state()
        except Exception:
            pass
    ...
    def erase_nand(self):
        ...
        try:
            ...
            if resume.get('operation') == 'ERASE':
                if not messagebox.askyesno(i18n.t('resume_found_title'), i18n.t('resume_erase_text')):
                    self.controller.clear_resume_state()
        except Exception:
            pass
